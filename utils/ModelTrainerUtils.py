...
class TrainerUtils:
    ...
    @staticmethod
    def log_result_mlflow(params, metrics, df_metrics, artifacts, run):
    ...
    #             mlflow.log_metrics(current_epoch_metrics, step=epoch)
        ...
        # TODO : log the dataset 
        #mlflow.log_input(dataset, context="training")

    @staticmethod
    def make_cross_val_df(df,idx,sw):
        if sw is None:
            return df.iloc[:idx]
        else:
    ...
    @staticmethod
    def get_start_end_idx(df, test_set_len=126):
        return df.shape[0]-test_set_len, df.shape[0]
